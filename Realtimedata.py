...
class TickStreamer(TWSClient):
    ...
    def newsubsucibe(self):
        contracts = [
            Stock('GOOG'),CFD('IBUS500')
         ]
        for contract in contracts:
            reqId = self.getReqId()
            self._reqId2Contract[reqId] = contract
            self.reqMktData(reqId, contract, genericTickList='', snapshot=False,regulatorySnapshot=False, mktDataOptions=[])
            # self.reqTickByTickData(reqId,contract,"BidSize",7,True)
